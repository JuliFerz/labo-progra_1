...
def capitalizar_palabras(text: str) -> str:
    ...
    if len(split_text) >= 1 and len(text) > 1:
        for word in split_text:
            split_text[split_text.index(word)] = word.capitalize()
        retorno = ' '.join(split_text)
    return retorno
...
def calcular_min_genero(lista: list, key: str, gen: str) -> dict:
    ...
    if (lista and re.search('^[fFmM]{1}$|^[nbNB]{2}$', gen)):
        heroe_min = primer_heroe_heroina(lista, gen)
        heroe_min[key] = float(heroe_min[key])
        for heroe in lista:
            heroe[key] = float(heroe[key])
            if heroe[key] < heroe_min[key] and heroe['genero'] == gen.upper():
                heroe_min = heroe
        retorno = heroe_min
    return retorno
...
def sumar_dato_heroe_genero(lista: list, key: str, gen: str) -> int:
    ...
    for el in lista:
        el[key] = float(el[key])
        if (type(el) == type({}) and el and el['genero'] == gen.upper()):
            suma += el[key]
            retorno = suma
...
def obtener_lista_de_tipos(lista: list, key: str) -> tuple:
    ...
    '''
    lista_tipo_datos = set([])
    for heroe in lista:
        dato_heroe = capitalizar_palabras(heroe[key]) or 'N/A'
        lista_tipo_datos.add(dato_heroe)
