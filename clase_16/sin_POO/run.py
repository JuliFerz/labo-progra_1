...
clock_fps = pygame.time.Clock() # lockear el while a x cantidad de FPS


while running:
    tiempo = clock_fps.tick(15) # lockear a 60 FPS
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_r:
                tablero.shuffle(tablero_juego)
                pass
            elif event.key == pygame.K_ESCAPE:
                running = False
            """ if event.key == pygame.K_RIGHT:
                cuadrado.move_ip(10, 0)
            if event.key == pygame.K_LEFT:
                cuadrado.move_ip(-10, 0)
            if event.key == pygame.K_DOWN:
                cuadrado.move_ip(0, 10)
